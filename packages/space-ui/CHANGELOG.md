...
- 536a1b7: Put the whole library on the spacing grid, and size type in rem.
...
- a18ca34: Add a charts section: `Chart`, `BarList` and `PieChart`.
...
- 3a58c72: Fix `Carousel`'s arrows and dots not keeping up when a breakpoint changes `--sp-carousel-view-count`.
...
- 5456874: Fix `Select`'s panel being trapped inside a Dialog, or any ancestor that clips or contains it.
  ...
  `Autocomplete`'s results panel had the same problem and gets the same fix.
